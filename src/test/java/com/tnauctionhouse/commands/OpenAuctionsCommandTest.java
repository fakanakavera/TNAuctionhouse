...
public class OpenAuctionsCommandTest {

	private TNAuctionHousePlugin plugin;
	private OpenAuctionsCommand command;

	@BeforeEach
	void setUp() {
		MockBukkit.mock();
		plugin = Mockito.mock(TNAuctionHousePlugin.class, Mockito.RETURNS_DEEP_STUBS);
		command = new OpenAuctionsCommand(plugin);
	}
	...
	void nonPlayerSenderGetsError() {
		...
		boolean result = command.onCommand(console, null, "auctions", new String[] {});
	...
	void missingPermission() {
		...
		Mockito.when(player.hasPermission("tnauctionhouse.auctions")).thenReturn(false);
		boolean result = command.onCommand(player, null, "auctions", new String[] {});
		assertEquals(true, result);
