...
public class SellOrdersGUI extends BaseOrdersGUI {

    public SellOrdersGUI(TNAuctionHousePlugin plugin, Player viewer, int page) {
        super(plugin, viewer, page);
    }

    @Override
    protected String getTitle() {
        return "Sell Orders - Page " + (page + 1);
    }

    @Override
    protected void populateItems() {
        List<SellOrder> pageOrders = plugin.getOrderManager().getSellOrdersPage(page, 45);
        int slot = 0;
    ...
    protected void handleClick(int rawSlot, ItemStack clickedItem, org.bukkit.event.inventory.ClickType clickType) {
        ...
        // Pagination
        if (rawSlot == 45) {
            if (page > 0) new SellOrdersGUI(plugin, viewer, page - 1).open();
            return;
        }
        if (rawSlot == 53) {
            new SellOrdersGUI(plugin, viewer, page + 1).open();
            return;
        }
        if (rawSlot == 49) {
		...
		new ConfirmPurchaseGUI(plugin, viewer, order, page, "ALL", null, null).open();
    }
}
