...
public class TypeFilteredSellOrdersGUI extends BaseOrdersGUI {

    private final ItemTypeCategory category;

    public TypeFilteredSellOrdersGUI(TNAuctionHousePlugin plugin, Player viewer, int page, ItemTypeCategory category) {
    ...
    protected String getTitle() {
        return "Sell Orders: " + category.name() + " - Page " + (page + 1);
    }

    @Override
    protected void populateItems() {
        List<SellOrder> pageOrders = plugin.getOrderManager().filterSellOrdersByCategory(category, page, 45);
        int slot = 0;
    ...
    protected void handleClick(int rawSlot, ItemStack clickedItem, org.bukkit.event.inventory.ClickType clickType) {
        ...
        if (rawSlot == 45) {
            if (page > 0) new TypeFilteredSellOrdersGUI(plugin, viewer, page - 1, category).open();
            return;
        }
        if (rawSlot == 53) {
            new TypeFilteredSellOrdersGUI(plugin, viewer, page + 1, category).open();
            return;
        }
        if (rawSlot == 49) {
            new FilterMenuGUI(plugin, viewer, false).open();
            return;
        }
		...
		new ConfirmPurchaseGUI(plugin, viewer, order, page, "CATEGORY", null, category).open();
    }
}
