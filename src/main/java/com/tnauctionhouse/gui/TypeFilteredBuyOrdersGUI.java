...
public class TypeFilteredBuyOrdersGUI extends BaseOrdersGUI {

    private final ItemTypeCategory category;

    public TypeFilteredBuyOrdersGUI(TNAuctionHousePlugin plugin, Player viewer, int page, ItemTypeCategory category) {
    ...
    protected String getTitle() {
        return "Buy Orders: " + category.name() + " - Page " + (page + 1);
    }

    @Override
    protected void populateItems() {
        List<BuyOrder> pageOrders = plugin.getOrderManager().filterBuyOrdersByCategory(category, page, 45);
        int slot = 0;
    ...
    protected void handleClick(int rawSlot, ItemStack clickedItem, org.bukkit.event.inventory.ClickType clickType) {
        ...
        if (rawSlot == 45) {
            if (page > 0) new TypeFilteredBuyOrdersGUI(plugin, viewer, page - 1, category).open();
            return;
        }
        if (rawSlot == 53) {
            new TypeFilteredBuyOrdersGUI(plugin, viewer, page + 1, category).open();
            return;
        }
        if (rawSlot == 49) {
		...
		new BuyOrderAmountPrompt(plugin, viewer, order, page, "CATEGORY", null, category).begin();
    }
}
